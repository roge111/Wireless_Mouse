#!/usr/bin/env python3
"""
Проект "Воздушная мышь" - версия 3.0 с улучшенной обработкой кнопок.
Версия 3.0: Исправлены ошибки именования, добавлен антидребезг на основе времени,
            структурированный код с секциями.
...
GPIO.setup(TOUCH_PIN, GPIO.IN)                      # Сенсор касания (ожидается HIGH при касании)
GPIO.setup(LEFT_BTN, GPIO.IN, pull_up_down=GPIO.PUD_UP)   # Кнопка левого клика (замыкает на GND)
GPIO.setup(RIGHT_BTN, GPIO.IN, pull_up_down=GPIO.PUD_UP)  # Кнопка правого клика (замыкает на GND)
...
# ------------------------------------------------------------
# ОСНОВНОЙ АСИНХРОННЫЙ ЦИКЛ СЕНСОРА И МЫШИ
# ------------------------------------------------------------
async def sensor_reader():
    ...
    # Инициализация датчика (один раз)
    if sox is None:
        i2c = board.I2C()
        ...
        sox.gyro_range = GyroRange.RANGE_125_DPS   # <-- ИСПРАВЛЕНО: было soxgyro_range

    # Переменные для антидребезга кнопок
    last_left_time = 0
    last_right_time = 0
    last_left_state = GPIO.HIGH   # не нажата
    last_right_state = GPIO.HIGH  # не нажата
    ...
    while not shutdown:
        try:
            ...
            # Калибровочные смещения (подставь свои)
            gyro[0] += 0.11011872144861299
            gyro[1] -= 0.24510967682695364
            gyro[2] += 0.030543261909900768

            # Сохранение последних данных для веб-интерфейса
            ...
            if last_left_state == GPIO.HIGH and left_now == GPIO.LOW:   # фронт: нажатие
            ...
            if last_right_state == GPIO.HIGH and right_now == GPIO.LOW: # фронт: нажатие
            ...
            if current_time - last_print_time >= 1.0:
                ...
                print(f"Iterations per second: {iteration_per_sec:,}")
                iteration_count = 0
...
# ------------------------------------------------------------
# WebSocket и HTTP серверы (без изменений)
# ------------------------------------------------------------
